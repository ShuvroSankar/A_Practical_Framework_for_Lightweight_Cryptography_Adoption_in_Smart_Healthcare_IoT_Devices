...
// Test
#define TEST_TAMPER_EVERY 10
static uint32_t txn = 0;
bool tamper = ((txn % TEST_TAMPER_EVERY) == 0);
...
void loop() {
  ...
  // Decide if we tamper this one (use +1 so the very first packet is clean)
  bool tamper = (TEST_TAMPER_EVERY > 0) && (((g_counter + 1) % TEST_TAMPER_EVERY) == 0);

  if (tamper) {
    // Packet layout: [4 counter][16 nonce][ciphertext][16 tag]
    const size_t off = 4 + 16;         // first byte of ciphertext
    // Safety: make sure ciphertext exists
    if (ct_len > 0) {
      pkt[off] ^= 0x01;                 // flip one bit in ciphertext
      Serial.println("** tampered packet **");
    }
  }
