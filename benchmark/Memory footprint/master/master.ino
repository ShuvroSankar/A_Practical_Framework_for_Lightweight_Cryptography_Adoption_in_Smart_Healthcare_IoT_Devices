...
#include <WiFi.h>
#include <esp_now.h>
#include <ASCON.h>   // ASCON Suite (rweather)
...
static bool printed_after_ascon = false;
// ------------------------------
...
void loop() {
  ...
  // One-time memory report right after first ASCON operation
  if (!printed_after_ascon && clen > 0) {
    REPORT_MEM("after_ascon");
    printed_after_ascon = true;
  }
  ...
  // Periodic memory stats during the stream (every ~2s)
