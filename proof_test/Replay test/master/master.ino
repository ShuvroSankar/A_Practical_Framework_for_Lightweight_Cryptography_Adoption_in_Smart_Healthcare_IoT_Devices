...
// put near top of the file
#define REPLAY_EVERY 20   
static uint32_t txn = 0;
...
// ---- assemble wire packet (you already have this above) ----
...
// ---- replay test: resend the SAME packet occasionally ----
bool do_replay = (REPLAY_EVERY > 0) && (((txn + 1) % REPLAY_EVERY) == 0);
if (do_replay) {
  // small gap to help ordering on-air (optional)
  delay(5);
  esp_now_send(SLAVE_MAC, pkt, pkt_len);   // exact duplicate
  Serial.println("** replayed current packet **");
}

// advance counters for NEXT packet
g_counter++;   // IMPORTANT: increment AFTER (re)send so both copies have same counter/nonce
txn++;
